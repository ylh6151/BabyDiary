...
public class GrowthHead extends Activity{
	DbHandlerGrow dbHandlerGrow;
	DbHandler dbHandler;
	String birthdate, gender,babyid;
	ArrayList<Double> headlist = new ArrayList<Double>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.growthhead);
	...
	private void genderpic(){
		...
		if (gender.equals("girl")){
			bgiv.setBackgroundResource(R.drawable.girlhead);
			//bgiv.setImageResource(R.drawable.girlweight);
		}
		else{
			bgiv.setBackgroundResource(R.drawable.boyhead);
	...
	private void drawpoint(){
		...
		LinearLayout.LayoutParams[] param = new LinearLayout.LayoutParams[headlist.size()];
		FrameLayout fl= (FrameLayout)findViewById(R.id.bgfl);
		LinearLayout[] ln = new LinearLayout[headlist.size()];
		ImageView[] point= new ImageView[headlist.size()];
		...
		for(int i = 0; i<headlist.size(); i++){
			...
			doty = ypoint - (ypoint/25*(headlist.get(i)))+(ypoint/25*29);//그래프 29cm부터 시작
	...
	private void select(){//리스트 불러오기
		...
		while(cursor.moveToNext()){
				...
				headlist.add(Double.parseDouble(cursor.getString(3)));
