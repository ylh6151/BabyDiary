...
public class Growthweight extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.growthweight);
		//maxwidth = chkmaxwidth();//원점
		//Toast.makeText(this, maxwidth, 3000);
		//mayheight= chkmaxheight();//원점 
	...
	private void genderpic(){
		...
		if (gender.equals("girl")){
			bgiv.setBackgroundResource(R.drawable.girlweight);
			//bgiv.setImageResource(R.drawable.girlweight);
		}
		else{
			bgiv.setBackgroundResource(R.drawable.boyweight);
			//bgiv.setImageResource(R.drawable.boyweight);
		}
	...
	private void drawpoint(){
		...
		LinearLayout.LayoutParams[] param = new LinearLayout.LayoutParams[weightlist.size()];
		FrameLayout fl= (FrameLayout)findViewById(R.id.bgfl);
		LinearLayout[] ln = new LinearLayout[weightlist.size()];
		ImageView[] point= new ImageView[weightlist.size()];
		...
		for(int i = 0; i<weightlist.size(); i++){
			...
			//param[i].setMargins(weightlist.get(i),daylist.get(i),0, 0);
	...
	private void select(){//리스트 불러오기
		...
		while(cursor.moveToNext()){
			/*	if((cursor.getString(1)).equals("0")){
					...
					weightlist.add(Double.parseDouble(cursor.getString(1)));
				...
				//daylist.add(Integer.parseInt(cursor.getString(4)));
			}//while 
			cursor.close(); 
	}
